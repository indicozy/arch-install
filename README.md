...
1. Запустите `base/base-uefi.sh` или `base/base-mbr.sh`, если у вас компьютер <2008 года то берите с `UEFI`
```
./base/base-uefi.sh
```
1. Настройте `mkinitcpio`
```
vim /etc/mkinitcpio.conf
```
В значении `MODULES=()` добавьте либо `i915` (Intel), либо `amdgpu` (AMD), либо `nvidia` (NVIDIA).

Запустите `mkinitcpio` 
```
mkinitcpio -p linux
```
1. Перезагрузите компьтер через:
```
exit
umount -a
reboot
```
1. Установите пакетный менеджер для AUR с помощью `AUR/paru.sh` или `AUR/yay.sh`, они особо не отличаются но для новичков рекомендую `yay`
```
./AUR/yay.sh
```
1. Установите один из Окружении в папке `DE`, после система сама перезагрузится
```
