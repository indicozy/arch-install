...
2. Проверьте, стоит ли у вас BIOS или UEFI
```bash
ls sys/firmware/efi
```
Если открывается, то у вас система запущена с UEFI.
...
Чтобы посмотреть новую таблицу ЖД:
```
p
```

2. Создайте раздела для **Linux**
```
n
ENTER
ENTER
...
3. Сохраните изменения и перезаписать ЖД:
